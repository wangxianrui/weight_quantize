...
import models.yolo.data.data_transforms as data_transforms


class COCODataset(Dataset):
    def __init__(self, data_root, list_path, img_size, is_training, is_debug=False):
        self.data_root = data_root
        with open(os.path.join(self.data_root, list_path), 'r') as file:
    ...
    def __getitem__(self, index):
        img_path = os.path.join(self.data_root, self.img_files[index % len(self.img_files)].rstrip())
        img = cv2.imread(img_path, cv2.IMREAD_COLOR)
        if img is None:
            raise Exception("Read image error: {}".format(img_path))
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img_id = int(os.path.splitext(img_path)[0][-7:])
        height, width, channels = img.shape

        label_path = os.path.join(self.data_root, self.label_files[index % len(self.img_files)].rstrip())
...
if __name__ == "__main__":
    dataloader = torch.utils.data.DataLoader(COCODataset("/home/wxrui/DATA/coco/coco/trainvalno5k.txt",
